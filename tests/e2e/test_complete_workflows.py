...
def run_workflow_command(args: list[str], input_text: str = "", cwd: str = None) -> tuple[int, str, str]:
    ...
    def test_complete_single_shot_workflow_with_configuration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            exit_code, stdout, stderr = run_workflow_command([
                ...
                "--prompt", "Respond with 'Configuration test successful'",
                "--no-tools",
                "--quiet"
            ])
            ...
            assert "configuration" not in stderr.lower() or "config" not in stderr.lower()
...
class TestConfigurationWorkflows:
    ...
    def test_configuration_override_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            exit_code, stdout, stderr = run_workflow_command([
                "--config", str(config_file),
                "--prompt", "Test configuration override",
...
class TestErrorRecoveryWorkflows:
    ...
    def test_graceful_error_recovery_workflow(self):
        ...
        test_scenarios = [
            ...
            {
                'name': 'invalid_config',
                'setup': lambda temp_path: (temp_path / "bad_config.yaml").write_text("invalid: yaml: ["),
                'args': lambda temp_path: ["--config", str(temp_path / "bad_config.yaml"), "--prompt", "test"]
            },
            # Missing required files
            {
                'name': 'missing_file',
                'setup': lambda temp_path: None,
                'args': lambda temp_path: ["--file", str(temp_path / "nonexistent.txt")]
            },
            # Invalid command combinations
            {
                'name': 'invalid_combination',
                'setup': lambda temp_path: (temp_path / "test.txt").write_text("test"),
                'args': lambda temp_path: ["--prompt", "test", "--file", str(temp_path / "test.txt")]
            }
        ]
        ...
        for scenario in test_scenarios:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                # Execute command
                try:
                    exit_code, stdout, stderr = run_workflow_command(scenario['args'](temp_path))
                    ...
                    if scenario['name'] == 'invalid_combination':
                        ...
                        assert exit_code in [0, 2], f"Scenario {scenario['name']} should handle mode conflict"
                    else:
                        # Other errors may exit with 0 but show error message
                        assert exit_code in [0, 1, 2], f"Scenario {scenario['name']} should handle error gracefully"
                    
                    assert stderr.strip() != "", f"Scenario {scenario['name']} should have error message"
                    
                    # Should not crash with stack trace
                    assert "Traceback" not in stderr, f"Scenario {scenario['name']} should not show stack trace"
                    
                except subprocess.TimeoutExpired:
                    pytest.fail(f"Scenario {scenario['name']} timed out - indicates hanging")
...
class TestCrossCommandWorkflows:
    ...
    def test_configuration_to_execution_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            exec_exit_code, exec_stdout, exec_stderr = run_workflow_command([
                "--config", str(config_file),
                "--prompt", "Configuration test execution",
                "--no-tools"
            ])
...
class TestPerformanceWorkflows:
    ...
    def test_cli_startup_performance_workflow(self):
        ...
        assert exit_code == 0
        assert startup_time < 2.0, f"CLI startup took {startup_time:.2f}s, expected < 2.0s"
    ...
    def test_command_processing_performance_workflow(self):
        ...
        exit_code, stdout, stderr = run_workflow_command([
            "--file", "/dev/null",  # Empty file input
            "--no-tools"
        ])
