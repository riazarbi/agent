...
from python_agent.agent import Agent, ModelError
...
class TestLiteLLMIntegration:
    ...
    def _skip_if_no_api_key(
        self, api_key: str | None, provider: str, env_var: str
    ) -> None:
        """Skip test if no API key is provided."""
        if not api_key:
            pytest.skip(
                f"Skipping {provider} API test: no API key provided (set {env_var})"
            )
    ...
    def test_anthropic_claude_integration_chat_completion(self):
        ...
        config = self._create_test_config(api_key, "claude-3-5-haiku-latest")
        agent = Agent(config)
    ...
    def test_api_error_handling_invalid_key(self):
        ...
        with pytest.raises(ModelError) as exc_info:
            agent.chat_completion([{"role": "user", "content": "This should fail"}])

        # Verify error message contains relevant information
        error_message = str(exc_info.value).lower()
        assert any(
            keyword in error_message
            for keyword in ["authentication", "invalid", "key", "unauthorized", "api"]
        )
    ...
    def test_api_error_handling_invalid_model(self):
        ...
        assert any(
            keyword in error_message
            for keyword in ["model", "invalid", "not found", "unsupported"]
        )
    ...
    def test_process_single_prompt_integration(self):
        ...
        response = agent.process_single_prompt(
            "Respond with 'Single prompt test successful'"
        )
    ...
    def test_custom_base_url_configuration(self):
        ...
        config = self._create_test_config(
            api_key, "gpt-3.5-turbo", base_url="https://api.openai.com/v1"
        )
        agent = Agent(config)
    ...
    def test_empty_content_handling(self):
        ...
        response = agent.chat_completion([{"role": "user", "content": "Just say ok"}])
...
class TestLiteLLMConversationWorkflows:
    ...
    def _get_openai_key(self) -> str | None:
        """Get Google Gemini API key for workflow tests."""
        return os.getenv("GEMINI_API_KEY")

    def _create_test_agent(self, api_key: str) -> Agent:
        """Create test agent with Google Gemini configuration."""
        config = get_default_config()
        config.update(
            {
                "api_key": api_key,
    ...
    def test_multi_turn_conversation_workflow(self):
        ...
        if not api_key:
            pytest.skip(
                "Skipping conversation workflow test: no Google Gemini API key (set GEMINI_API_KEY)"
            )
    ...
    def test_conversation_with_session_management(self):
        ...
        if not api_key:
            pytest.skip(
                "Skipping session workflow test: no Google Gemini API key (set GEMINI_API_KEY)"
            )
    ...
    def test_error_recovery_workflow(self):
        ...
        if not api_key:
            pytest.skip(
                "Skipping error recovery test: no Google Gemini API key (set GEMINI_API_KEY)"
            )
