...
def run_cli_command(args: list[str], input_text: str = "", timeout: int = 10) -> CLIResult:
...
class TestCLIIntegrationBasicFunctionality:
    ...
    def test_cli_single_shot_mode_with_simple_prompt(self):
        ...
        result = run_cli_command([
            "--prompt", "Hello, respond with 'Test response'",
            "--no-tools"
        ])
...
class TestCLIIntegrationConfigurationHandling:
    ...
    def test_cli_configuration_file_error_handling_for_invalid_yaml(self):
        ...
        try:
            ...
            assert "configuration" in result.stderr.lower() or "yaml" in result.stderr.lower()
    ...
    def test_cli_configuration_file_missing_file_error_handling(self):
        ...
        # CLI may exit with 0 but should show error message
        assert result.exit_code in [0, 1]
        assert "not found" in result.stderr.lower() or "does not exist" in result.stderr.lower() or "agent error" in result.stderr.lower()
...
class TestCLIIntegrationCommandLineOptions:
    ...
    def test_cli_tool_flags_allow_tools_and_no_tools(self):
        ...
        # Test --allow-tools
        result_allow = run_cli_command([
            "--prompt", "test prompt",
            "--allow-tools"
        ])
        
        # Test --no-tools
        result_no_tools = run_cli_command([
            "--prompt", "test prompt", 
            "--no-tools"
        ])
    ...
    def test_cli_mode_exclusivity_validation(self):
        ...
        try:
            ...
            assert "cannot use" in result.stderr.lower() or "together" in result.stderr.lower()
...
class TestCLIIntegrationOutputFormats:
    ...
    def test_cli_error_message_quality_and_actionability(self):
        ...
        try:
            ...
            assert "error" in error_msg or "failed" in error_msg or "dictionary" in error_msg
